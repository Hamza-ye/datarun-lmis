...
| `ledger_inventory_events` | Event Store | Append-only, immutable event log |
| `ledger_stock_balances` | Event Store | CQRS read-model for current stock |
...
- `ledger_inventory_events`: Append-only. No `UPDATE` or `DELETE` permitted.
- `ledger_stock_balances`: `UNIQUE(node_id, item_id)`. OCC via `version` column.
- `ledger_idempotency_registry`: `UNIQUE(source_event_id)`.
- `ledger_inventory_events` insert + `ledger_stock_balances` update always in a **single DB transaction**.

## Notable Columns Added by Audit

| Column | Table | Purpose |
|---|---|---|
| `adjustment_reason` | `ledger_inventory_events` | Sub-type for ADJUSTMENT / STOCK_COUNT events. See [Event Store → Adjustment Reasons](event-store.md#adjustment-reasons). |
| `occurred_at` | `ledger_inventory_events` | Business Time (when it happened in the field). Distinct from `created_at` (System Time). See [Event Store → Temporal Ordering](event-store.md#temporal-ordering). |
...
### `ledger_inventory_events.transaction_type`
