...
def mock_contract_dsl():
    return {
        ...
        "ingress": {"trigger_path": "body.event_type", "trigger_value": "RECEIPT"},
        "destination": {"method": "POST", "url": "http://example-ledger.test/ingest"},
        "dictionaries": {
            "external": {"node_map": {"namespace": "hf_to_dhis2", "on_unmapped": "DLQ"}}
        },
...
async def test_layer3_egress_success(
    mock_request, db_session: AsyncSession, mock_contract_dsl
):
    ...
    # Execute Layer 3
    # ------------------
    processed_count = await AdapterWorker.process_egress_batch(
        batch_size=10, session=db_session
    )
    ...
    assert log_entry.destination_http_code == 200


@pytest.mark.asyncio
@patch("httpx.AsyncClient.request")
async def test_layer3_egress_domain_rejection(
    mock_request, db_session: AsyncSession, mock_contract_dsl
):
    ...
    # Execute Layer 3
    processed_count = await AdapterWorker.process_egress_batch(
        batch_size=1, session=db_session
    )
    assert processed_count == 1
...
@pytest.mark.asyncio
@patch("httpx.AsyncClient.request")
async def test_layer3_egress_transport_retry(
    mock_request, db_session: AsyncSession, mock_contract_dsl
):
    import httpx
