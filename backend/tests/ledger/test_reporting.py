...
@pytest.mark.asyncio
async def test_get_balances_rbac_filtering(
    async_client: AsyncClient, seed_reporting_data
):
    """
    ...
    def override_actor_clinic_a():
        return ActorContext(
            actor_id="test", roles=["ledger_viewer"], allowed_nodes=["CLINIC_A"]
        )
...
@pytest.mark.asyncio
async def test_get_balances_global_admin(
    async_client: AsyncClient, seed_reporting_data
):
    ...
    def override_actor_admin():
        return ActorContext(
            actor_id="admin", roles=["GLOBAL_ADMIN"], allowed_nodes=["GLOBAL"]
        )
...
async def test_get_history_forbidden(async_client: AsyncClient, seed_reporting_data):
    ...
    def override_actor_clinic_c():
        return ActorContext(
            actor_id="test", roles=["ledger_viewer"], allowed_nodes=["CLINIC_C"]
        )
