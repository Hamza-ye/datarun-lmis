...
async def test_happy_path_transfer(db_session, seeded_warehouse):
    ...
    registry = await InTransitService.process_dispatch(
        db_session, dispatch_cmd, dest_node_id="CLINIC_1"
    )
    ...
    warehouse_bal = (
        (
            await db_session.execute(
                select(StockBalance).where(StockBalance.node_id == "WAREHOUSE")
            )
        )
        .scalars()
        .first()
    ...
    clinic_bal = (
        (
            await db_session.execute(
                select(StockBalance).where(StockBalance.node_id == "CLINIC_1")
            )
        )
        .scalars()
        .first()
    ...
    # 2. Receipt
    receipt_cmd = base_command(
        TransactionType.RECEIPT, 50, "RECV_1", str(registry.transfer_id)
    )
    receipt_cmd.node_id = "CLINIC_1"

    registry_updated = await InTransitService.process_receipt(
        db_session, receipt_cmd, str(registry.transfer_id)
    )
    await db_session.commit()
...
async def test_partial_receipt_discrepancy(db_session, seeded_warehouse):
    ...
    dispatch_cmd.node_id = "WAREHOUSE"
    registry = await InTransitService.process_dispatch(
        db_session, dispatch_cmd, "CLINIC_1"
    )
    await db_session.commit()

    receipt_cmd = base_command(
        TransactionType.RECEIPT, 40, "RECV_2", str(registry.transfer_id)
    )
    receipt_cmd.node_id = "CLINIC_1"
...
async def test_auto_close_stale_transfers(db_session, seeded_warehouse):
    ...
    registry.auto_close_after = datetime.datetime.now(
    ...
    stmt = select(InTransitRegistry).where(
        InTransitRegistry.transfer_id == registry.transfer_id
    )
...
@pytest.mark.asyncio
async def test_internal_dlq_on_auto_close_error(
    db_session, seeded_warehouse, monkeypatch
):
...
async def test_loss_in_transit(db_session, seeded_warehouse):
    ...
    # 2. Loss Event (ADJUSTMENT with adjustment_reason='LOSS_IN_TRANSIT')
    loss_cmd = base_command(TransactionType.ADJUSTMENT, 50, "LOSS_EVENT_1")
    loss_cmd = loss_cmd.model_copy(
        update={"node_id": "WAREHOUSE", "adjustment_reason": "LOSS_IN_TRANSIT"}
    )

    registry_updated = await InTransitService.process_loss(
        db_session, loss_cmd, str(registry.transfer_id)
    )
    await db_session.commit()

    # 3. Verify Registry is LOST_IN_TRANSIT
    assert registry_updated.status == InTransitStatus.LOST_IN_TRANSIT
    ...
    stmt = select(InventoryEvent).where(
        InventoryEvent.source_event_id == "LOSS_EVENT_1"
    )
    ...
    assert loss_audit_event.transaction_type == "ADJUSTMENT"
    assert loss_audit_event.adjustment_reason == "LOSS_IN_TRANSIT"


@pytest.mark.asyncio
async def test_partial_not_auto_closed(db_session, seeded_warehouse):
    """Documented invariant: PARTIAL transfers must NOT be auto-closed.
    Only OPEN transfers with zero receipts should be auto-closed.
    """
    # 1. Dispatch 50
    dispatch_cmd = base_command(TransactionType.TRANSFER, 50, "DISP_PARTIAL")
    dispatch_cmd.node_id = "WAREHOUSE"
    registry = await InTransitService.process_dispatch(
        db_session, dispatch_cmd, "CLINIC_1"
    )
    ...
    # 2. Partial receipt of 20 (status → PARTIAL)
    receipt_cmd = base_command(
        TransactionType.RECEIPT, 20, "RECV_PARTIAL", str(registry.transfer_id)
    )
    receipt_cmd.node_id = "CLINIC_1"
    await InTransitService.process_receipt(
        db_session, receipt_cmd, str(registry.transfer_id)
    )
    await db_session.commit()
    ...
    reg = (await db_session.execute(stmt)).scalars().first()
    assert reg.status == InTransitStatus.PARTIAL

    # 3. Backdate the deadline so it WOULD be eligible if not excluded
    reg.auto_close_after = datetime.datetime.now(
        datetime.timezone.utc
    ) - datetime.timedelta(days=1)
    await db_session.commit()

    # 4. Run auto-close cron — should NOT close PARTIAL transfers
    closed_count = await InTransitService.auto_close_stale_transfers(db_session)
    await db_session.commit()

    assert closed_count == 0  # PARTIAL excluded!

    # Status should still be PARTIAL, not STALE_AUTO_CLOSED
    reg_final = (await db_session.execute(stmt)).scalars().first()
    assert reg_final.status == InTransitStatus.PARTIAL
