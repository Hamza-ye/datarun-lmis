    ...
    events = (
        (
            await db_session.execute(
                select(InventoryEvent).order_by(InventoryEvent.created_at)
            )
        )
        .scalars()
        .all()
...
@pytest.mark.asyncio
async def test_adjustment_reason_persisted(db_session):
    """Verifies that adjustment_reason is carried through to the InventoryEvent record."""
    # Seed with stock
    cmd1 = base_command(TransactionType.RECEIPT, 100, "SEED")
    await EventStoreService.commit_command(db_session, cmd1)

    # ADJUSTMENT with reason
    cmd2 = LedgerCommand(
        source_event_id="ADJ_DAMAGE_1",
        version_timestamp=1,
    ...
    assert event.adjustment_reason == "DAMAGE"
    assert event.quantity == -10

    bal = (await db_session.execute(select(StockBalance))).scalars().first()
    assert bal.quantity == 90


@pytest.mark.asyncio
async def test_adjustment_reason_null_for_non_adjustment(db_session):
    """Verifies that adjustment_reason is None for non-ADJUSTMENT events."""
    cmd = base_command(TransactionType.RECEIPT, 50, "RECEIPT_1")
