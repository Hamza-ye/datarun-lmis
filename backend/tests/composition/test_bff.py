...
async def test_bff_node_overview_success(db_session: AsyncSession):
    ...
    # Kernel
    db_session.add(
        NodeRegistry(uid=node_id, name="Test Clinic", node_type="HF", code="TC01")
    )

    # Ledger
    db_session.add(StockBalance(node_id=node_id, item_id="ITEM1", quantity=100))
    ...
    actor = ActorContext(
        actor_id="test_user", roles=["system_admin"], allowed_nodes=["GLOBAL"]
    )

    # 3. Call Service
    result = await CompositionService.get_node_overview(
        db_session, actor.allowed_nodes, node_id
    )
