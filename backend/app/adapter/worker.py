...
class AdapterWorker:
    ...
    async def process_mapping_batch(batch_size: int = 50, session: AsyncSession = None):
        ...
        async with async_session_maker() as new_session:
            return await AdapterWorker._do_process_mapping_batch(new_session, batch_size)
    ...
    async def _do_process_egress_batch(session: AsyncSession, batch_size: int):
        stmt = select(AdapterInbox).where(
        ...
        for item in inbox_items:
            ...
            assert item.mapped_payload is not None, "LAYER 3 INVARIANT FAILURE: mapped_payload is NULL."
            assert item.mapping_version is not None, "LAYER 3 INVARIANT FAILURE: mapping_version is NULL."
            ...
            async with httpx.AsyncClient() as client:
                for cmd in commands:
                    try:
                        ...
                        if session:
                            await AdapterWorker.insert_egress_log_async(log_data, session=session)
                        else:
                            asyncio.create_task(AdapterWorker.insert_egress_log_async(log_data))
