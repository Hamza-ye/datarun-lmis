...
class CompositionService:
    ...
    @staticmethod
    async def get_node_overview(db: AsyncSession, actor: ActorContext, node_id: str) -> Dict[str, Any]:
        """
        ...
        async def fetch_node():
            try:
                stmt = select(NodeRegistry).where(NodeRegistry.uid == node_id, NodeRegistry.valid_to == None)
        ...
        async def fetch_adapter():
            try:
                ...
                stmt = select(AdapterInbox).where(
                    AdapterInbox.status == InboxStatus.RECEIVED
                ).order_by(AdapterInbox.created_at.desc()).limit(5)
                res = await db.execute(stmt)
                items = res.scalars().all()
                results["pending_sync"] = {
                    "status": "ok", 
                    "count": len(items),
                    "latest": [{"id": str(i.id), "received_at": i.created_at} for i in items]
                }
            except Exception as e:
