...
class PolicyCreate(BaseModel):
    policy_key: str
    applies_to_node: Optional[str] = (
        None  # NULL = global, 'type:<code>' = node type, specific UID otherwise
    )
    applies_to_item: Optional[str] = (
        None  # NULL = all items, 'category:<name>' = category, specific item_id otherwise
    )
    config: Dict[str, Any]
...
async def list_commodities(
    actor: ActorContext = Depends(get_current_actor), db: AsyncSession = Depends(get_db)
):
...
async def create_or_update_policy(
    ...
    actor.require_role("system_admin")
    # Upsert logic based on coordinates (handling NULL correctly)
    node_filter = (
        SystemPolicy.applies_to_node == payload.applies_to_node
        if payload.applies_to_node
        else SystemPolicy.applies_to_node.is_(None)
    )
    item_filter = (
        SystemPolicy.applies_to_item == payload.applies_to_item
        if payload.applies_to_item
        else SystemPolicy.applies_to_item.is_(None)
    )
...
async def resolve_nodes(
    ...
    stmt = select(NodeRegistry.uid, NodeRegistry.name).where(
        NodeRegistry.uid.in_(unique_ids), NodeRegistry.valid_to == None
    )
    result = await db.execute(stmt)
...
async def create_node(
    ...
    stmt = select(NodeRegistry).where(
        NodeRegistry.uid == payload.node_id, NodeRegistry.valid_to == None
    )
    if (await db.execute(stmt)).scalars().first():
        raise HTTPException(
            status_code=400, detail="Active node with this UID already exists"
        )
...
async def update_node(
    ...
    if requires_history_split:
        ...
        if payload.name:
            active_node.name = payload.name
        if payload.node_type:
            active_node.node_type = payload.node_type
        if payload.meta_data:
            active_node.meta_data = payload.meta_data
...
async def historical_topology_correction(
    ...
    stmt_check = select(NodeRegistry).where(
        NodeRegistry.uid == node_id, NodeRegistry.parent_id == payload.new_parent_id
    )
    ...
    stmt = (
        ...
        .with_for_update()
    )  # ROW LEVEL LOCK FOR IDEMPOTENCY
