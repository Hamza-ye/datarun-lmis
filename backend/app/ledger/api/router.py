...
gatekeeper_router = APIRouter(
    prefix="/api/ledger/gatekeeper", tags=["Ledger Gatekeeper"]
)
...
async def submit_ledger_command(
    ...
    if idem_result.action == "IGNORE":
        ...
        return {
            "status": "IGNORED",
            "message": idem_result.reason,
            "existing_summary": idem_result.existing_summary,
        }
        ...
        await GatekeeperService.stage_command(
            db,
            reversal_command,
            "Symmetry of Governance: Reversal of modified transaction",
        )
        await db.commit()
        return {
            "status": "STAGED",
            "message": "Reversal of prior transaction requires manual approval before proceeding.",
        }

    # Dummy Threshold Policy (Normally resolving via Area F PolicyResolver)
    requires_approval = (
        command.quantity >= 1000
        or command.transaction_type == TransactionType.ADJUSTMENT
    )

    if requires_approval:
        # Route to Area E (Gatekeeper)
        await GatekeeperService.stage_command(
            db, command, "System Policy Threshold Exceeded"
        )
        await db.commit()
        ...
        if command.transaction_type == TransactionType.TRANSFER:
            await InTransitService.process_dispatch(
                db,
                command,
                dest_node_id=command.metadata.get("dest_node_id", "UNKNOWN"),
            )
        elif (
            command.transaction_type == TransactionType.RECEIPT and command.transfer_id
        ):
...
async def resolve_staged_command(
    ...
    action_type = (
        ...
        else ApprovalActionType.REJECT
    )
    payload = SupervisorActionPayload(
        actor_id=actor.actor_id, action=action_type, comment=action_data.comment
    )
...
async def get_stock_balances(
    ...
    balances = await ReportingService.get_balances(db, actor.allowed_nodes, node_id)
    return balances


@ledger_router.get(
    "/history/{node_id}/{item_id}", response_model=List[LedgerHistoryResponse]
)
async def get_inventory_history(
    ...
    """
    actor.require_node_access(node_id)
    history = await ReportingService.get_history(
        db, actor.allowed_nodes, node_id, item_id, limit
    )
    return history
...
async def list_transfers(
    ...
    if node_id:
        ...
        stmt = stmt.where(
            or_(
                InTransitRegistry.source_node_id == node_id,
                InTransitRegistry.dest_node_id == node_id,
            )
        )
    elif "GLOBAL" not in actor.allowed_nodes:
        stmt = stmt.where(
...
@ledger_router.post(
    "/transfers/{transfer_id}/receive", status_code=status.HTTP_201_CREATED
)
async def receive_transfer(
    ...
    """
    if (
        "GLOBAL" not in actor.allowed_nodes
        and payload.node_id not in actor.allowed_nodes
    ):
        raise HTTPException(status_code=403, detail="No access to this node.")
    ...
    idem_result = await IdempotencyService.check_or_register_command(
        db, receipt_command
    )
    if idem_result.action == "IGNORE":
        await db.commit()
    ...
    result = await InTransitService.process_receipt(
        db, receipt_command, str(transfer_id)
    )

    from app.ledger.models.idempotency import IdempotencyRegistry, IdempotencyStatus

    stmt_idem = select(IdempotencyRegistry).where(
        IdempotencyRegistry.source_event_id == receipt_command.source_event_id
    )
    ...
    if idem_record:
        ...
        idem_record.result_summary = {
            "status": "COMMITTED",
            "event_id": receipt_command.source_event_id,
        }
